...
현재 구현은 필기시험 답안 입력, 제출 스냅샷, 자동 채점, 결과 저장 흐름을 중심으로 한다. 이 문서는 그 흐름을 유지하면서 계수형 R&R, 계량형 R&R, 교육 검증, SQLite 원본 저장소로 확장하기 위한 목표 처리 설계다.
...
- 원본 저장소는 SQLite다.
- 기존 CSV/브라우저 저장 흐름은 SQLite 전환 전까지 호환 계층으로 유지하고, 전환 후에는 계량형 측정값 import 편의와 결과 export/백업 용도로만 사용한다.
...
7. 기준 미달 결과는 정식 인증서 발행 차단 사유가 된다.
...
6. 기준 미달 결과는 정식 인증서 발행 차단 사유가 된다.

## 6. 교육 이수 처리

교육 이수는 `TrainingRecord`로 관리한다. 교육명, 이수 일자, 이수 시간, 증빙 경로, 확인자, 확인 시각, 상태를 저장한다. 필수 교육 누락, 최소 시간 미달, 증빙 누락은 정식 인증서 발행 차단 사유다.
...
필수 평가 또는 필수 증빙이 하나라도 미충족이면 `approved`가 될 수 없고 정식 인증서 발행을 차단한다.
