...
현재 구현은 필기시험 중심 UI에 더해 SQLite 기반 자격 인증 API를 제공하는 로컬 웹 애플리케이션이다. R&R, 교육, 자격 판정, 인증서 이력은 로컬 HTTP API와 SQLite 저장소 경계에서 동작한다.
...
| `qualification` | 자격 유형, 필수 평가 계획, 평가 세션 관리 | 구현됨(API/SQLite) |
| `attribute-rr` | 계수형 R&R 세트, 샘플, 판정 원본, 결과 계산 | 구현됨(API/SQLite) |
| `variable-rr` | 계량형 R&R 계획, 측정 원본, range 방식 결과 계산 | 구현됨(API/SQLite) |
| `training` | 교육 이수와 증빙 충족 여부 관리 | 구현됨(API/SQLite) |
| `certification` | 발행 가능성 검증, 자격 판정, 인증서 발행 이력 | 구현됨(API/SQLite) |
| `sqlite-store` | SQLite 원본 저장과 조회 경계 | 구현됨 |
| `csv-io` | CSV import/export 보조 기능과 수식 인젝션 방지 | 기존 리포트 기능에서 전환 |
| `audit-log` | 제출, 계산, 판정, 발행, 취소, 재발행 감사 이력 | 구현됨(API/SQLite) |
...
- 기존 CSV/브라우저 저장 흐름은 호환·비상용 계층으로 취급하며, 원본 저장 이후에는 내보내기·백업 보조 기능으로 제한한다.
